...
### D-066 — Separate the Build Week challenge capture from production live authority

- Date: 2026-07-20
- Status: `ACCEPTED`
- Milestone: M7/M10
- Context: The official Build Week submission requires a working project built with Codex and GPT-5.6, a public sub-three-minute YouTube demo with audio, a repository and README, and the primary `/feedback` session ID. It does not require PolicyTwin's stronger production cgroup-v2, dynamic-container, direct Responses API, deployment-attestation, or `verify:live` proof. The production gate is structurally fail-closed on the current Windows/cgroup-v1 host, while the deadline still requires a truthful and reproducible challenge artifact.
- Options considered:
  1. weaken `verify:live` or reinterpret partial offline evidence as production success;
  2. submit only the recorded fixture and describe Codex use without a bounded executable capture;
  3. preserve every production gate and add a separate explicit `LOCAL_CHALLENGE` profile plus a separate challenge submission package and checker.
- Decision: Use option 3. `LOCAL_CHALLENGE` is non-root-exported, unreachable from product routes, pinned to the Codex-supported GPT-5.6 Sol identifier `gpt-5.6-sol`, disabled without `POLICYTWIN_LOCAL_CHALLENGE_APPROVED=1`, and uses the existing Codex login with the official provider forced, MCP configuration cleared, web search disabled, network disabled inside the fixture task, and approval set to never. It works only in a disposable copy of the seeded baseline. Model-written source is admitted to host typecheck/test only after a strict pure-AST allowlist; the test file may differ only by enabling the three existing skipped regressions. The runner then executes two fixed commands, retains their full receipts, evaluates all 41 accepted cases outside the fixture tree, retains every result, requires zero drift and a distinct read-only review, rechecks clean-commit provenance, and emits exactly three hash-bound files under `artifacts/challenge-evidence/`. Every production, direct-Responses, isolation, release-attestation, and deployment claim is hard-coded false. The independent challenge submission package lives under `artifacts/challenge-submission/`; it does not replace or weaken `artifacts/submission/`, `pnpm verify:live`, or `pnpm submission:check`.
- Evidence: `src/codex/sdk-adapter.ts`, `scripts/local-challenge.mjs`, `scripts/local-challenge-contract.mjs`, `schemas/local-challenge-run.v1.schema.json`, `tests/unit/local-challenge-profile.test.mjs`, `scripts/challenge-submission-check.mjs`, `artifacts/challenge-submission/`, and the 2:48 challenge-video capture/render pipeline. A Codex GPT-5.6 independent release review found the Windows preflight, evidence-parity, provenance, and video-timeline defects that were corrected before this decision was recorded.
- Consequences: PolicyTwin has a deadline-appropriate path that can truthfully demonstrate Build Week Codex/GPT-5.6 collaboration without manufacturing production proof. The local checker may pass while explicitly listing owner-controlled external actions; the release checker remains nonzero until a license, public repository URL, public YouTube URL, and validated challenge evidence exist.
