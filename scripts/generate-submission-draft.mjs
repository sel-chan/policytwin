...
write(
  submissionDirectory,
  "whats-next.md",
  `# What's next\n\n${draft}\n\nComplete a fresh GPT-5.6 run, build the non-privileged OS worker, OpenAI-only egress proxy, and no-network immutable verifier behind the existing authenticated supervisor transport, connect live run state to the existing six-screen workspace, finish the container/deployment, select a project license, record the demo, complete owner eligibility declarations, and submit. Post-challenge expansion remains outside the MVP.`,
);
write(
  submissionDirectory,
  "technologies.txt",
  `${draft}\nImplemented offline: TypeScript, Node.js, pnpm, Next.js 16, React 19, Playwright with Chrome, Node.js built-in SQLite persistence and durable RPC replay state, OpenAI Responses adapter contract, TLS 1.3 mutual-authentication transport, bounded signed external-worker supervisor/RPC contract, Rego source generation, OPA 1.18.2, split web/worker/verifier Dockerfiles and static launch-plan checks, Git.\nPlanned/unverified: fresh GPT-5.6 Structured Output, immutable worker/verifier image execution, the OS executor and OpenAI-only egress proxy, live Codex SDK work, dynamic SQLite container persistence, and deployment.`,
);
write(
  submissionDirectory,
  "openai-and-codex-usage.md",
  `# OpenAI and Codex usage\n\n${draft}\n\nStatus: NOT_RUN_LIVE.\n\nThe intended live path uses GPT-5.6 through the Responses API for strict semantic interpretation and delegates one complete repair run to an external supervisor. Its concrete TLS 1.3 transport mutually authenticates CA-chained and certificate-pinned peers, fixes the server name and ALPN, bounds one canonical request/response frame, and persists one-use request-ID/nonce state in SQLite. The host RPC contract additionally binds the fixed write/command/corpus policy, host-known baseline and signed final tree manifests, trusted supervisor signature, separate immutable verification workspace, and teardown receipt. Split worker/verifier Dockerfiles and a fixed non-root/read-only/two-file-overlay/no-network-verifier plan now exist but have not run. Current transport tests use an explicit signed \`FAIL\` executor double; no OS executor, OpenAI-only egress proxy, or live Codex turn exists. No submission may claim live OpenAI/Codex work until \`pnpm verify:live\` captures fresh request/run evidence.`,
);
write(
  submissionDirectory,
