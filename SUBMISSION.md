...
Worker RPC v2 and CPU evidence schema v2 are now prepared offline with separate signature and mTLS downgrade domains, client execution binding, deterministic role/attempt bindings, distinct live-purpose key material, durable replay, a strictly ordered global monotonic event transcript, and closed success/failure outcomes. The contract recomputes overlap, verifier ordering, role samples, arithmetic, containment state, and evidence hashes; legacy proof v1, static fake objects, nullable `cpuProof` receipts, and contradictory outcomes are rejected. An internal synthetic state-machine producer now derives parsed unsigned candidates and exercises ordering, overage, identity drift, cleanup failure, input-snapshot, abort, and overflow paths. Every candidate is frozen with `liveClaim:false` and `passSigningEligible:false`, and self-declared Linux provenance is rejected. This does not add current live proof: the loopback v2 integration signs only typed pre-execution `FAIL`, every success or observed failure fixture is synthetic, the generic supervisor still refuses PASS, and runtime-observation flags remain false. No real Linux adapter or dedicated lifecycle, Docker execution, `cpu.stat` observation, containment action, hard cap, bounded overshoot, model call, or Codex repair may be claimed from this checkpoint.

The schema-v12 static container contract retains the hardened non-live observer and separately records new non-admitted Linux building blocks: a host/role one-shot start-barrier protocol bundled in all three role images, a serial non-privileged lifecycle harness with quiescent teardown sampling, and a fixed-frame repository-owned C helper/client boundary for RAW clock, pidfd, exact cgroup-v2 binding, freeze, kill, quiescence, and release. The latest helper source compiled with strict C17 warnings in local WSL2, but that distribution exposes tmpfs rather than cgroup v2 and the helper therefore failed closed before handshake. This is not Docker or Linux runtime evidence: no immutable role image, real cgroup, Docker-owned system adapter, finalized-evidence issuer, signed PASS, or live gate admission exists.

Schema v12 keeps the internal CPU-adapter/final-result identity guard and 28-stage order while adding only non-admitted barrier, non-privileged lifecycle-harness, and native-helper source layers. Module-private identity still rejects raw evidence, harness diagnostics, helper frames, synthetic candidates, non-live observations, copies, wrappers, and JSON-round-tripped values; there is deliberately no finalized-evidence issuer. This is not runtime evidence: Docker-owned Linux adaptation, cgroup-v2 execution, signer capability admission, PASS signing, and the live gate remain disabled.
