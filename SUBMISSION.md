...
The schema-v14 static container contract retains the hardened non-live observer and separately records a concrete private Linux construction: a deeply frozen factory-issued supervisor lifecycle plan, owner-created and independently inspected worker-internal/outbound networks, internally derived exact role plans and barrier invocations, a Docker-CLI-backed owner with independent reobservation and removal/absence receipts, a full fixed-frame helper role session, a Docker/cgroup system adapter, and a dedicated private lifecycle. A Docker create/network command that may have produced a side effect but fails before a trusted ID is parsed can recover only one exact-name, exact-label owned resource using an independent cleanup signal; an empty, ambiguous, or foreign observation remains a sticky failure. The normal adapter path keeps each role held while Docker is observed, helper bind captures the baseline, Docker is reobserved, the cached baseline is accepted, and the barrier releases. It then performs serial sampling, containment when required, quiescent final sampling, per-role Docker removal and cgroup release, followed by helper stop and owned-network removal. Emergency helper termination is allowed only after every Docker role and network is absent. The latest helper source compiled with strict C17 warnings in local WSL2, but that distribution exposes tmpfs rather than cgroup v2 and the helper therefore failed closed before handshake. This is implementation evidence, not Docker or Linux runtime evidence: no immutable role image or real cgroup exercised the path, and no finalized-evidence issuer, signed PASS, or live gate admission exists.

Schema v14 keeps the internal CPU-adapter/final-result identity guard and required lifecycle order while adding a factory-issued sealed lifecycle plan, owner-created private networks, internally derived Docker role plans, owned containers, one-shot bind/removal receipts, helper sessions, the system adapter, and the private lifecycle wrapper. Module-private identity still rejects raw evidence, harness diagnostics, helper frames, synthetic candidates, non-live observations, copied plans, wrappers, and JSON-round-tripped values; there is deliberately no finalized-evidence issuer. This is not runtime evidence: cgroup-v2 execution, signer capability admission, PASS signing, and the live gate remain disabled.
