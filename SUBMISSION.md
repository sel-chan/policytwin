...
- Submission status: `ACCOUNT_SUBMISSION_CONFIRMED_STRICT_RELEASE_GATES_INCOMPLETE`
- Last rules check: `2026-07-21 00:04:17 +09:00 — all three official OpenAI and Devpost pages refreshed directly; the official challenge minimum remains distinct from PolicyTwin's production live gate`
...
- Confirmation ID/URL: `1080298-policytwin` / `https://devpost.com/software/policytwin`
- Confirmation evidence path: `artifacts/submission/submission-confirmation.png`; `artifacts/submission/submission-confirmation.json`
...
- `artifacts/demo/policytwin-demo.mp4` is the visually reviewed public upload: 2:48, 1920×1080 H.264, 48 kHz stereo AAC, synchronized eight-cue narration/captions, and SHA-256 `190f585b6cb6023ada28b13c46a6d3dccff082acbe024bbaaf7b29d49042786e`.
...
- The repository, replacement 2:48 YouTube video, and canonical Devpost entry are publicly reachable. Anonymous YouTube oEmbed and Devpost HTML checks resolve the exact replacement video and updated story, and the owner confirmed public playback/audio on `2026-07-21`. `pnpm challenge:submission:release` performs anonymous Git, YouTube oEmbed, and Devpost probes. The official challenge handoff does not replace or weaken `artifacts/submission/`, `pnpm verify:live`, or `pnpm submission:check`; those remain the stricter production-release target.
...
- The signed-in Devpost management page for submission `1080298-policytwin` visibly reports `PolicyTwin`, `SUBMITTED`, and `5/5 steps done`. A privacy-cropped 941×720 PNG and exact SHA-256-bound confirmation object are stored under `artifacts/submission/`; anonymous checks also verify the canonical public page `https://devpost.com/software/policytwin`, the corrected story, 41/41 zero-drift result, and replacement video `https://youtu.be/h7o1vXmWC-M`. This closes the account-confirmation gap only. The strict production release still fails closed on the explicitly separate live deployment, direct Responses API, eligible Linux cgroup-v2 worker, finalized attestation, and `verify:live` gates. The bounded local Codex GPT-5.6 capture remains non-production challenge evidence and does not promote those claims.
...
A separate internal v2 repair-execution core now binds a canonical active request and its input/policy/execution hashes to the existing offline cartography, repair, fixed typecheck/test, complete-corpus, and review sequence. Its structural port is labeled `UNVERIFIED_INJECTED_BACKEND`, and sensitive content anywhere in the completed report is rejected. It returns only a deeply frozen `UNSIGNED_WORKER_EXECUTION_CANDIDATE`; live, PASS-signing, and external-settlement eligibility remain false. The module is absent from the root export and is not connected to the worker entrypoint, Docker target, real SDK constructor, Worker RPC response, finalized evidence, or repair-run settlement, so it is reusable orchestration contract evidence rather than a Codex run.

The accepted 41-case loop is now extracted into a second internal pre-authority module. It checks an active canonical v2 request around each case, derives canonical PolicyIR plus unverified input/result binding hashes, requires typecheck and test to preserve one caller-observed tree, and preserves exact case order while safely classifying invalid or throwing decisions. Its evaluator, attempt/run, command transcript, tree digests, and injected clock are explicitly caller supplied and unverified. The frozen `UNSIGNED_VERIFIER_CORPUS_CANDIDATE` cannot parse as policy verification evidence or a v2 response, cannot be consumed as a validated external run, and keeps live/signing/settlement flags false. TypeScript AST inspection with module resolution rejects supported production references from `src/`, `scripts/`, or `app/`, including commented/query-suffixed re-exports, package/tsconfig mappings, non-literal direct loaders, and common indirect `require` loaders. The contract calls this static graph evidence, not runtime proof. No Docker verifier, immutable snapshot, server-owned receipt, replay authority, model call, or live proof is claimed.

A separate internal verifier exchange now stages the next non-runtime authority boundary: exact source/build manifests are locally reinspected, a one-use 256-bit capability authenticates a request/image/corpus/tree/command-bound HMAC receipt, and a sealed SQLite store persists issue/consume/poison state, request-attempt tombstones, uniqueness, and clock high-water across restart. PASS permits one bound structural review and FAIL attempt 1 permits one fresh-snapshot retry. The result remains `BOUND_NOT_RUNTIME_FINALIZED`; capability delivery is in-process, review is a caller-supplied bound echo, and no verifier entrypoint, Worker RPC response, signer, settlement, web executor, model call, or live proof consumes it. Static/common-loader inspection is a regression guard, not runtime or arbitrary-code-generation proof.
