...
The schema-v11 static container contract retains the hardened non-live cgroup observer. Exact Docker cgroup components, private directory-FD/device/inode identity, bounded no-follow reads, full uint64 `bigint` CPU parsing, `cgroup.events populated=0`, one-shot handle closure, counter-regression rejection, original-cgroup release, and sticky cleanup-action failures are covered by offline tests and source-tamper checks. This is not Linux runtime evidence: no cgroup filesystem or Docker daemon was observed, the role-local baseline still occurs after start, the real D-040 capability adapter/start barrier/containment lifecycle remains absent, and no observer result is accepted by a signer or live gate.

Schema v11 now adds an internal CPU-adapter identity scaffold and a 28-stage dedicated success-order contract. Module-private identity rejects raw evidence, synthetic candidates, non-live observations, copies, wrappers, and JSON-round-tripped values; there is deliberately no finalized-evidence issuer. The stage order requires barrier hold before cgroup bind/baseline, release only afterward, and Docker/cgroup cleanup plus controller stop before finalization. This is not runtime evidence: the Linux adapter, barrier runtime, RAW clock, polling, containment, signer capability admission, PASS signing, and live gate remain disabled.
