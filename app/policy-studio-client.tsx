  ...
  return (
    <WorkspaceShell active="studio" eyebrow={`Workspace / isolated v${currentVersion.version}`} title="Policy Studio" summary="Source language, structured meaning, and executable rules stay in one review surface." actions={<><StatusPill tone={isDraft || unresolved > 0 ? "warn" : "ok"}>SQLite v{currentVersion.version}</StatusPill><button className="primary" type="button" disabled>Interpret with GPT-5.6</button></>}>
      ...
      <div className="two-column studio-grid">
        <section className="panel policy-source"><div className="panel-heading"><div><span className="kicker">Natural language contract</span><h2>Seeded SaaS refund policy</h2></div><span className="mono">{currentVersion.sourceText.length} chars</span></div><div className="policy-paper">{currentVersion.sourceText}</div><div className="notice"><strong>Evidence boundary</strong><span>{isDraft ? `The v${currentVersion.version} source is persisted, but its candidate meaning contradicts G02. Prior v${latestValidatedVersion.version} proof remains authoritative.` : "This PolicyIR began as a recorded fixture. A fresh GPT-5.6 call is still required before submission."}</span></div></section>
