...
- `pnpm container:verify`, `pnpm worker:verify`, and `pnpm egress:verify` are separate non-live Docker prerequisites for web health, worker/verifier isolation, and the TLS-only egress path. The latter two require inspected-ID ownership and the schema-v11 cgroup-v2 observer boundary: exact Docker membership, private descriptor/device/inode identity, `populated=0` descendant quiescence, initial-PID absence, original-cgroup release, exact uint64 CPU arithmetic, and sticky cleanup-action failures. This observer has not run on Linux, still measures after Docker start, and is not the private live-evidence adapter. The TLS probe writes no HTTP, but proxy outbound traffic is not measured. A fake-only three-role CPU ledger now proves static aggregation and receipt-validation order, while explicitly claiming no enforcement or hard limit. The live gate rejects that proof and any unbound boolean until a signed real-Linux cgroup proof exists.
...
- Schema v11 adds an internal CPU-adapter capability scaffold and the required 28-stage lifecycle order. Adapter identity uses a private runtime set, the finalized-evidence guard has no issuer, and raw/synthetic/non-live/copied values are rejected. The order requires barrier hold → cgroup bind → baseline → release for every role and Docker/cgroup release → controller stop → finalization. The Linux adapter, start-barrier runtime, RAW clock, polling, containment, finalized result, signer admission, and PASS remain unimplemented.
