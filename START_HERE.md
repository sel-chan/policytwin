...
- `pnpm container:verify`, `pnpm worker:verify`, and `pnpm egress:verify` are separate non-live Docker prerequisites for web health, worker/verifier isolation, and the TLS-only egress path. Schema v14 retains the post-start non-live observer and adds a separate private construction path: a deeply frozen factory-issued supervisor lifecycle plan, owner-created and independently inspected worker-internal/outbound networks, internally derived exact role plans, one-shot host/role barriers, Docker-owned container and reobservation capabilities, a full fixed-frame native-helper role session, a Docker/cgroup system adapter, and a dedicated private lifecycle. The older static Docker gates do not invoke this path, the helper has not run against cgroup v2, and no result can reach a signer. The TLS probe writes no HTTP, proxy outbound traffic is not measured, and the live gate still rejects every fake, unsigned, or structurally shaped CPU claim.
...
- Schema v14 keeps the private adapter/final-result identity boundary and required order, rejects copied lifecycle plans, makes the owner create and prove absence of both private networks, and implements the Docker-owned plan/owner/helper/system-adapter/dedicated-lifecycle construction behind private identities. It is not connected to the static Docker driver or signer and has no dynamic runtime evidence. The finalized-evidence guard still has no issuer; cgroup-v2 runtime verification, finalized results, signer admission, and PASS remain unimplemented.
