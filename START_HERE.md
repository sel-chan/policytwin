...
- `pnpm helper:verify`, `pnpm container:verify`, `pnpm worker:verify`, and `pnpm egress:verify` are separate non-live prerequisites for the native artifact, web health, worker/verifier isolation, and TLS-only egress path. Schema v15 retains the post-start observer and adds a lifecycle-v3 private construction that seals helper artifact image/source/build/binary identities with the role plan, owned networks, barriers, Docker reobservations, helper session, system adapter, and dedicated lifecycle. The local two-build report is explicitly unpinned and non-promotable; the artifact gate uses no pull or build network and currently fails before Docker because the immutable compiler image is unset. The reachable Docker 29.1.5 daemon uses cgroup v1, so worker/egress also require a different Linux cgroup-v2 supervisor after immutable prerequisites are available; an early host preflight now rejects that ineligible environment before builds without replacing per-container proof. The older static Docker gates do not invoke this path, the helper has not run against cgroup v2, and no result can reach a signer. The TLS probe writes no HTTP, proxy outbound traffic is not measured, and the live gate still rejects every fake, unsigned, or structurally shaped CPU claim.
- Worker RPC v2 now requires CPU evidence schema v2: one request-bound global monotonic event transcript for a success or non-CPU execution failure, plus closed pre-execution, controller-failure, observed-overage-contained, and containment-incomplete branches. An internal synthetic producer exercises the state machine and returns only a frozen unsigned, non-live candidate wrapper whose current signing eligibility is false; it rejects a port that self-declares Linux provenance. The enclosed raw evidence remains parser-valid for contract testing and is not provenance or signer authorization. The legacy role-local proof v1 and nullable `cpuProof` receipt cannot be promoted. The generic v2 supervisor remains fail-only; the loopback integration signs typed pre-execution `FAIL`, synthetic unit evidence is not runtime evidence, and the live gate rejects every available result until the concrete private construction is dynamically verified on Linux Docker/cgroup v2 and a separate final-result issuer is deliberately added.
- Schema v15 keeps the private adapter/final-result identity boundary and required order, rejects copied lifecycle plans, binds the same-FD helper hash to the sealed artifact identity, makes the owner create and prove absence of both private networks, and implements the Docker-owned plan/owner/helper/system-adapter/dedicated-lifecycle construction behind private identities. It is not connected to the static Docker driver or signer and has no artifact-image or dynamic runtime evidence. The finalized-evidence guard still has no issuer; host installation, cgroup-v2 runtime verification, finalized results, signer admission, and PASS remain unimplemented.
