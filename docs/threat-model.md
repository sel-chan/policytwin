...
4. Server to repair worker: the planned transport must mutually authenticate a single-run RPC carrying only validated policy evidence, opaque request identity, immutable digests/manifests, closed IDs, and resource policy; the current client enforces the interface precondition, but no mTLS/socket-ACL transport exists yet.
...
| Arbitrary command execution | SDK phases reject command events; host live construction and commands reject; the RPC contract fixes one request, two writable files, two command IDs, a separate immutable verification workspace, an authentication precondition, streamed canonical byte caps, baseline/final manifest comparison, signed supervisor teardown, OpenAI-proxy-only SDK egress, and zero fixture-process network | Implement and verify the actual mTLS/socket-ACL transport, supervisor, worker image, mount/network/cgroup policy, immutable harness, and process-tree reaping before enabling any SDK turn |
...
| Denial of service | Per-command 30-second timeout and 2 MiB process buffer; at most two repair attempts; each SDK phase has abort, event-count, event-byte, final-response, prompt, file-count, file-byte, and review-patch limits; RPC responses require a 1..4 MiB declared raw length before body access, 64 KiB chunks, at most 1,024 chunks, exact length, and abort-aware iteration | The actual transport must enforce the same limits before socket-buffer allocation; SDK stderr/single-JSONL-line hard limits, shared request rate limits, and externally enforced CPU/memory/PID/process-tree termination remain mandatory |
...
This is not a completed release security review. SQLite restart behavior, browser-session isolation, same-origin/CSRF mutation rejection, bounded body reads, checksum-pinned local OPA, production Next build, deterministic archive construction, local Chrome E2E, dependency audit, static scans, fake-stream SDK controls, streamed signed RPC parsing/replay/request/tree-manifest binding checks, and the static web-container contract are tested. The host live factory still always rejects; no actual SDK is constructed by the RPC path, no web route invokes repair, and the host command runner rejects live mode. Production storage, authenticated identity, distributed quotas, the external supervisor/worker, actual mTLS or socket ACL, egress proxy, OS mount/resource enforcement, live signing-key custody, immutable Node image digest, dynamic container runtime, hosting, and deployment secrets remain untested. `artifacts/evidence/security-review.md` therefore stays `NOT_RUN`, and the partial proof package remains `FAIL`.
