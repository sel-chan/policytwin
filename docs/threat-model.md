...
| Denial of service | Per-command 30-second timeout and 2 MiB process buffer; at most two repair attempts; each SDK phase has abort, event-count, event-byte, final-response, prompt, file-count, file-byte, and review-patch limits; the TLS supervisor checks 1 MiB/4 MiB frame declarations before body accumulation, tracks pre-handshake sockets, limits connections and one active repair, rejects partial/trailing frames, aborts on deadlines, and waits for executor settlement on shutdown | Network-edge connection/rate limits, SDK stderr/single-JSONL-line hard limits, shared admission control, and externally enforced CPU/memory/PID/process-tree termination remain mandatory |
| Supply-chain compromise | Exact dependency lock, production audit, checksum-pinned OPA, and a web Dockerfile with no mutable base-image fallback | Verify and record the immutable Node base-image digest, then rerun release-platform license/audit/image checks |
...
This is not a completed release security review. SQLite restart behavior, browser-session isolation, same-origin/CSRF mutation rejection, bounded body reads, checksum-pinned local OPA, production Next build, deterministic archive construction, local Chrome E2E, dependency audit, static scans, fake-stream SDK controls, signed RPC bindings, real loopback mTLS peer authentication/framing/cancellation, durable replay rejection, and the static web-container contract are tested. The host live factory still always rejects; no actual SDK is constructed by the RPC path, no web route invokes repair, and the host command runner rejects live mode. Production storage, authenticated identity, distributed quotas, the OS-isolated worker, worker image, egress proxy, mount/resource enforcement, live signing-key custody, immutable Node image digest, dynamic container runtime, hosting, and deployment secrets remain untested. `artifacts/evidence/security-review.md` therefore stays `NOT_RUN`, and the partial proof package remains `FAIL`.
