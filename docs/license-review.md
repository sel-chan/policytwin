...
The repository now has six direct production dependencies locked to exact versions: `@openai/codex-sdk` 0.144.3, `next` 16.2.10, `openai` 6.46.0, `react` 19.2.7, `react-dom` 19.2.7, and `zod` 4.4.3. The official OPA 1.18.2 executable is downloaded into ignored `.tools/` for local verification and is not committed.
