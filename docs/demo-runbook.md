...
After credentials, an owner-selected license, Docker, deployment permission, and external live-purpose/attestation keys are available, the runbook must add the implemented `pnpm verify:live` workflow, a Linux cgroup-v2 controller producing fresh Worker RPC v2 CPU evidence, deliberate under-budget, over-budget, identity, controller, containment, and cleanup-failure runs, dynamic evidence-v2 validation, application URL, deployed health check, live browser path, Codex diff/review evidence, signing step, and rollback/redeploy commands. Those steps are not currently runnable.
