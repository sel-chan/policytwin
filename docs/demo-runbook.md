...
Schema v11 retains the non-live observer checks introduced in v10 and adds only the private CPU-adapter/final-result identity scaffold plus the required dedicated success-stage order. It cannot be run: there is no finalized-evidence issuer, concrete Linux adapter, start barrier, RAW clock, serial poller, containment controller, or signer admission. Do not substitute the synthetic producer, the non-live observer, or a parser-valid CPU evidence object. The future run must demonstrate barrier-held → cgroup-bound → baseline-recorded → barrier-released for every role and must complete Docker/cgroup cleanup plus controller stop before finalization.
