...
- `container:check` validates the daemon-free, split web/worker/verifier contracts and confirms every live/dynamic flag remains false;
...
`pnpm worker:verify` separately requires the contract's exact worker/verifier build-input hashes, an immutable Node base already present in the local daemon, and an internal-only `policytwin-worker-internal` network. On a Docker-eligible run it rebuilds the current TypeScript runtime plan, builds both images from the checked-in Dockerfiles and current repository context without an external mutable syntax frontend, records the resulting immutable image IDs plus Docker server version in the dynamic report, and removes the temporary tags after the run. It prepares a disposable baseline plus an exact two-file repair overlay, mounts the canonical fixture read-only with only `src/refund.ts` and `tests/refund.test.mjs` writable, and runs the static worker preflight as UID 10001 with a read-only root, dropped capabilities, no-new-privileges, and fixed PID/memory/CPU limits. It then reconstructs a separate verification tree from the canonical baseline and those two overlay contents, records the copied paths and all three baseline/overlay/reconstructed content bindings, and mounts that tree read-only into a UID 10002 verifier with `--network none`, an empty credential environment, and tmpfs-only `dist`/`tmp`. The verifier runs fixed TypeScript/test commands. The command is non-live: it does not connect to OpenAI, prove the still-unimplemented egress proxy, construct the SDK, or promote evidence. It currently fails before Docker because the immutable Node base is unset; Docker/BuildKit release verification remains separate.
