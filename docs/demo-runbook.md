...
After credentials, an owner-selected license, Docker, deployment permission, and external live-purpose/attestation keys are available, the runbook must add the implemented `pnpm verify:live` workflow, a Linux cgroup-v2 controller producing a fresh Worker RPC v2 proof, deliberate over-budget and cleanup-failure tests, evidence-v2 validation, application URL, deployed health check, live browser path, Codex diff/review evidence, signing step, and rollback/redeploy commands. Those steps are not currently runnable.
