...
- `container:check` validates the daemon-free, split web/worker/verifier/egress contracts and confirms every live/dynamic flag remains false;
...
`pnpm worker:verify` separately requires the contract's exact worker/verifier/egress build-input hashes, an immutable Node base already present in the local daemon, and an internal-only `policytwin-worker-internal` network. On a Docker-eligible run it still builds and smokes only the worker and verifier. It does not yet build or run `Dockerfile.egress-proxy`. The daemon-free lifecycle plan already fixes an unexposed UID 10003 proxy container, external read-only TLS/provider mounts, the outbound and internal networks, and explicit create/start/wait/logs/stop/remove operations. The prepared broker code and fake-upstream integration test are not dynamic network proof. The command remains non-live and currently fails before Docker because the immutable Node base is unset.
