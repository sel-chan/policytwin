...
- `container:check` validates the daemon-free split images, request/nonce-bound Docker v2 ownership contract, ID-only supervisor driver, and separate TLS-only gate while confirming every live/dynamic flag remains false;
...
Schema v12 retains those checks and adds only non-admitted implementation pieces: role-side and host-side one-shot barrier code, a non-privileged serial lifecycle harness, and a fixed-frame native helper/client source boundary. The default static entrypoints still bypass the barrier, the helper has not run against cgroup v2, and no Docker-owned system adapter or finalized-evidence issuer exists. Do not substitute the harness, native source, synthetic producer, non-live observer, or parser-valid object for runtime evidence. A future run must dynamically demonstrate barrier-held → cgroup-bound → baseline-recorded → barrier-released for every role, quiescent final samples, Docker/cgroup release, controller stop, and only then private finalization.
...
After credentials, an owner-selected license, Docker, deployment permission, and external live-purpose/attestation keys are available, the runbook must add the implemented `pnpm verify:live` workflow, a private-capability Linux cgroup-v2 adapter plus dedicated bounded-cleanup lifecycle producing fresh Worker RPC v2 CPU evidence, deliberate under-budget, over-budget, identity, controller, containment, and cleanup-failure runs, dynamic evidence-v2 validation, application URL, deployed health check, live browser path, Codex diff/review evidence, signing step, and rollback/redeploy commands. Those steps are not currently runnable.
