...
- a schema-v11 boundary that retains the non-live observer introduced in v10 and adds a private CPU-adapter capability scaffold plus dedicated lifecycle contract. Compile-time brands are backed by private adapter/final-result identity sets, the scaffold is frozen and non-live, and no finalized-result issuer or registrar exists. The 28-stage success order requires each role's held barrier before cgroup bind and baseline, serial observation with identity revalidation, sticky cleanup, Docker/cgroup release, controller stop, and only then finalization. No concrete Linux adapter or live signer consumes this contract;
