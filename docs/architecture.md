...
- a schema-v12 boundary that retains the v10 observer and v11 private final-result guard while adding non-admitted implementation layers. Every role image bundles a one-shot launcher; the host pre-creates and owns fixed receipt payload/commit slots in a non-replaceable directory, the trusted role launcher closes the payload before committing its SHA-256, and the host snapshots its own inodes before one concurrency-guarded release. This POSIX design avoids requiring the role to own the host's files, but actual cross-UID bind-mount permissions and absence of surviving writable FDs remain dynamic verification obligations. A `NON_PRIVILEGED_TEST_PORT` harness implements serial role-local RAW/sample ordering, quiescent final sampling, teardown-tail accounting, abort checks, sticky cleanup, and bounded forced-termination diagnostics, but cannot issue evidence. A repository-owned C helper/client defines fixed frames and owns future `CLOCK_MONOTONIC_RAW`, pidfd, exact `openat2` cgroup-v2 identity, freeze, kill, quiescence, and release primitives out of process. Strict WSL compilation passed, but cgroup-v2 runtime, Docker ownership integration, helper packaging, final issuance, signer admission, and live verification remain false;
