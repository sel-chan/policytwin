...
- separate static worker/verifier Dockerfiles and a deterministic run-plan contract that fix non-root users, read-only roots, dropped capabilities, resource ceilings, a read-only baseline plus exactly two writable file overlays, an internal worker network, and a credential-free `network=none` verifier. These are configuration evidence only until immutable images run under Docker.
...
- the web, worker, and verifier Dockerfiles and daemon-free static checks exist, but their image digests, dynamic container health/isolation, worker egress proxy, live browser run, and deployment do not.
