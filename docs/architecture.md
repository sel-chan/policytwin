...
- a schema-v14 private Linux construction that retains the v10 observer, v11 final-result guard, and v12 barrier/helper boundaries. One deeply frozen factory-issued supervisor lifecycle plan closes the immutable images, exact role configuration, run labels, limits, mounts, and intended network topology; copied or caller-shaped plans are rejected by object identity. A real Docker-CLI-backed owner preflights names, creates and independently inspects the worker-internal and outbound networks, derives role plans only from those observed IDs, creates roles without starting them, independently verifies ownership/runtime identity, connects networks, starts held roles, issues single-use reobservation receipts, and confirms container plus network removal through inspect/list/name absence. Create failures that may follow side effects use a separate bounded cleanup signal and can recover only one exact-name, exact-label owned resource; empty, foreign, or ambiguous observations remain sticky failures. The full helper session binds only owner receipts, enforces role state and one session-global RAW clock, and performs sample/freeze/kill/quiescence/release operations. For each held role, helper bind captures the baseline between the first and second Docker observations; only after the second observation succeeds does the lifecycle accept that cached baseline and release the barrier. The normal path then serially samples, contains when required, takes a final quiescent sample, removes each role's Docker resource, releases its cgroup, stops the helper after all roles, and removes both owned networks before closing the barrier and owner. The emergency path permits forced helper termination only after all Docker roles and networks are independently absent. The dedicated lifecycle wrapper accepts only the exact private adapter. The static gates do not invoke this construction, strict WSL compilation is not runtime proof, and cgroup-v2 execution, cross-UID barrier/FD behavior, helper packaging, final issuance, signer admission, and live verification remain false;
