...
- The installed Codex CLI 0.144.0 reports an existing ChatGPT login. After owner approval, one attempt failed before model use on login-stream parsing; a second reached the bounded Codex phases but failed closed before evidence promotion because Windows retained the isolated temporary Codex state database; two later attempts stopped at the project-pinned 0.144.3 CLI's missing `gpt-5.6` model metadata. No partial challenge evidence was promoted. The project SDK and bundled CLI are now upgraded and hash-pinned at 0.144.6 while the global login remains read-only source state. The SDK classifies the exact metadata fallback as a non-fatal item; only one exact `item.completed` diagnostic per phase is admitted for the local challenge backend, every other error remains fatal, final completion/agent/schema/filesystem checks remain mandatory, and observed phases are retained as closed evidence codes.
...
| Unit tests | PASS | `pnpm test` | 449/449 pass, including the exact non-production local challenge profile, exact one-per-phase GPT-5.6 metadata-fallback diagnostic admission/evidence, cross-process run-lock/tombstone/reviewed-retirement coverage, schema v1/v2→v3 migration, durable retirement/restart, OPA budgets, release, media, container, and RPC coverage | 2026-07-20 13:16 +09:00 |
...
| Static container contract | PASS | `pnpm container:check` | worker `cbf12961…`, verifier `0497698d…`, egress `e0b926c2…`, and helper `cab5ad2b…` match the checked contract after the Codex SDK 0.144.6 and exact metadata-fallback diagnostic update | 2026-07-20 13:12 +09:00 |
