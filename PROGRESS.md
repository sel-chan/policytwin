...
Implement the next fail-closed slice behind the private adapter boundary: an immutable role-side one-shot start barrier, a host-owned release controller contract, and a dedicated lifecycle state machine that orders held barrier, identity binding, baseline, release, serial observation, containment, Docker/cgroup release, controller stop, and finalization eligibility. The Windows host cannot dynamically validate Linux cgroup or Docker behavior, so this checkpoint must keep finalized-evidence issuance, Worker RPC v2 PASS signing, live admission, and every dynamic-runtime claim disabled.

### Starting failing condition

Starting HEAD is clean `main` at `8b768abaa7699f3bce758d507652c9738af43523`. The final prior-checkpoint `pnpm verify` passed 280 unit, 57 integration, 22 eval, 3 browser, the production build, the then-current schema-v11 static contract, 342-file clean-copy, and security/history checks, then failed only at the intentionally unmet owner license and exact 29-item submission gate. The private adapter was still a non-runtime scaffold: no start barrier implementation, host release capability, owned-container lifecycle, raw-clock system adapter, running poller, containment actuator, or finalized-evidence issuer existed. The existing static Docker lifecycle starts role code immediately and finalizes fake accounting before cleanup; the public observer samples only a post-start, non-live path. Neither may be promoted.

### Planned actions

- [x] Re-read all control documents, confirm clean `main`, recheck PowerShell/Node/pnpm/Docker availability, and map the static Docker, synthetic producer, non-live observer, private adapter, and 28-stage contract boundaries.
- [x] Obtain a read-only lifecycle integration review; it confirms the existing static driver and post-start observer cannot satisfy the dedicated lifecycle and recommends a separate private implementation.
- [x] Add failing unit tests for one-shot role barrier semantics, private host-only release authority, exact stage transitions, serial operation exclusion, sticky containment/cleanup failure, and no finalization before all owned resources are released.
- [x] Implement the role-side barrier launcher and host-side barrier controller with run-scoped capabilities, bounded reads/writes, no-follow path checks, same-FD freezing, atomic one-shot release, exact argv/`NODE_OPTIONS` controls, and immutable image bundling without changing the static entrypoints.
- [x] Implement a separate non-privileged dedicated lifecycle harness plus fixed-frame native Linux helper/client boundary for RAW clock, pidfd/cgroup identity, per-role serial samples, quiescent teardown-tail accounting, containment primitives, and bounded cleanup diagnostics. Do not connect the fake harness or raw helper boundary to the private finalized-evidence issuer.
- [x] Advance the static contract to schema v12 and update Decision D-043 plus submission/architecture/threat/limitations/runbook wording only for implemented structure; keep Docker integration, real system-adapter availability, dynamic verification, evidence issuance, PASS signing, and live admission false.
...
- Starting HEAD: `8b768abaa7699f3bce758d507652c9738af43523`; clean `main` before this ledger update.
- Environment recheck at `2026-07-16 15:18 +09:00`: Node `v22.22.2`, pnpm `11.9.0`; Docker CLI remains installed but the Docker Desktop Linux engine pipe is absent. No Linux/Docker dynamic execution is available on this host.
- Prior authoritative replay: the 14:58 `pnpm verify` passed every implemented gate through the production build and exited 1 only for the owner `LICENSE` and exact 29-item non-final submission gate.
- Current failing condition: the role images have no pre-execution barrier and the only Docker driver starts default entrypoints before cgroup binding/baselining. The final-evidence private identity registry intentionally has no issuer.
- Failing test evidence: the first focused run failed with `ERR_MODULE_NOT_FOUND` for both new modules before implementation.
- Barrier result: three role images bundle `scripts/role-start-barrier.mjs`; the default static entrypoints remain unchanged. The controller and prepared-role authorities use private identity maps. Before any container starts, the host creates and owns fixed `held.json` and `held.commit.json` slots, then removes directory write permission so the role may fill but not replace those paths. The trusted bundled role launcher closes and syncs the payload before writing its SHA-256 commit; the host snapshots its own inodes, validates the commit binding, concurrency-guards the release transition, and only then emits a binding-matched release from the separate container-read-only control mount. This is a reviewed POSIX permission design, not observed cross-UID bind-mount evidence; a future Linux gate must also prove no writable receipt FD survives. Exact targets and empty `NODE_OPTIONS` prevent loader pre-execution in the intended dedicated invocation.
- Lifecycle result: the only executable orchestrator is explicitly `NON_PRIVILEGED_TEST_PORT`. It checks cancellation around every execution operation, records one RAW timestamp per role sample, serializes all reads, recomputes aggregate `bigint` usage including quiescent teardown tails, and returns only frozen non-live/non-final diagnostics. Cleanup uses independent signals, force-termination settlement bounds, sticky failures, and never issues finalized evidence; an operation that remains unsettled after forced termination rejects the run and blocks every terminal result.
- Native boundary result: the repository-owned C helper defines fixed 24-byte/256-byte-bounded frames, strict sequence and role identity, exact Docker cgroup components, `CLOCK_MONOTONIC_RAW`, pidfd revalidation, `openat2` beneath/no-link/no-cross-mount resolution, cgroup-v2 FD identity, uint64 sampling, freeze/kill/quiescence/release, and EOF containment. The private client hashes a non-writable open FD and executes that same FD through `/proc/self/fd/3`; it exposes no Docker bind authority in this checkpoint.
- Linux compile evidence: the latest helper source compiled in existing WSL2 `Ubuntu-Hermes` with `cc -std=c17 -O2 -Wall -Wextra -Werror -Wpedantic -fstack-protector-strong -D_FORTIFY_SOURCE=3 -fPIE -pie` (exit 0, no network). Runtime handshake failed closed at helper initialization because both available WSL distributions expose tmpfs (`statfs` magic `1021994`) rather than cgroup v2 at `/sys/fs/cgroup`; this is not runtime evidence.
- Focused regression evidence: schema-v12 static container check passes; dedicated lifecycle coverage includes teardown-tail overage, abort-before-release, identity drift, independent cleanup, cooperative timeout termination, and rejection of every terminal result for uncooperative late settlement. Post-gate reviews found and drove three barrier fixes: host-owned cross-UID payload/commit slots, `HELD -> RELEASING` concurrency admission, and prepare-mode-before-final-rename publication with nonce/prepublish failure becoming sticky `FAILED`. The final result passes lint, typecheck, 20/20 focused barrier/adapter/container cases, 300/300 full unit assertions, 57/57 integration, 22/22 eval, 3/3 browser, 352-file clean-copy, 352-file/323-text-file security, schema-v12 static container, demo, and production build. Existing-writer revocation and actual cross-UID bind mounts remain explicit dynamic blockers rather than static claims. Final read-only re-review reports no remaining P0/P1. `pnpm verify` remains fail-closed only for the owner `LICENSE` and exact 29-item submission gate.
- Hermetic tool-resolution evidence: the first full integration replay exposed global TypeScript 5.8.3 through `tsc.cmd`, which rejected the project-required `--ignoreConfig`; project TypeScript 6.0.3 supports it. `scripts/process.mjs` now prefers repository-local `node_modules/.bin` shims, two regression tests pass, the focused fixture integration passes 5/5, and the full integration suite passes 57/57.
- Transient replay note: one pre-acceptance combined replay reported 296/297 unit assertions, but its output filter did not preserve the failing case. The same authoritative unit command then passed once and three more consecutive times at 297/297 without a code change. Checkpoint acceptance therefore requires a later clean combined replay instead of treating the transient run as a pass.
- Expected-failure evidence: all three dynamic container gates stop before Docker at the unset immutable Node base; `verify:live` stops before network at missing `OPENAI_API_KEY` and `CODEX_MODEL`; the license gate reports `OWNER_DECISION_REQUIRED`; submission remains exactly 29 unmet requirements.
...
| Lint | PASS | `pnpm lint` | schema-v12 barrier/helper/harness, local-tool resolution, source-tamper checks, generated submission notes, cgroup observer, RPC, and repository static checks pass | 2026-07-16 18:03 +09:00 |
| Typecheck | PASS | `pnpm typecheck` | strict TypeScript 6.0.3 covers the barrier controller, non-privileged lifecycle, native-helper protocol/client, private capability scaffold, v1/v2 RPC, application, and policy-engine boundaries | 2026-07-16 18:03 +09:00 |
| Unit tests | PASS | `pnpm test` | 300/300 pass, including host-precreated receipt inode preservation, commit-hash mismatch, concurrent release, prepublish failure, nonce failure, barrier/lifecycle/helper boundaries, schema-v12, repository-local executable resolution, and existing RPC/Docker/cgroup regressions | 2026-07-16 18:03 +09:00 |
| Integration tests | PASS | `pnpm test:integration` | 57/57 serial fixture, mTLS v1/v2, PASS-signing rejection, typed FAIL signing, replay restart, OPA, evidence, and persistence assertions pass with project TypeScript 6.0.3 | 2026-07-16 18:03 +09:00 |
| Browser tests | PASS | `pnpm test:e2e` | 3/3 production standalone Chrome tests; six views, archive, v1-v5 writes, isolation/capacity/expiry, focus, and 390px layout pass | 2026-07-16 18:03 +09:00 |
| Prompt/eval suite | PASS | `pnpm eval` | 22/22 offline/recorded evals pass with schema-v12 prepared-boundary wording and explicit runtime/live exclusions | 2026-07-16 18:03 +09:00 |
| Production build | PASS | `pnpm build` | Next.js 16 Turbopack standalone build and strict TypeScript pass with the internal non-exported modules | 2026-07-16 18:03 +09:00 |
| Offline full verification | FAIL_EXPECTED | `pnpm verify` | every implemented post-fix schema-v12 gate passes: 300 unit, 57 integration, 22 eval, 3 browser, 352-file clean copy, 352-file/323-text-file security plus Git history, static container, demo, and build; exit 1 is only owner `LICENSE` plus exact 29-item submission gate | 2026-07-16 18:03 +09:00 |
| Fresh live integration | FAIL | `pnpm verify:live` | fail-closed before network at missing `OPENAI_API_KEY` and `CODEX_MODEL`; no fresh evidence exists | 2026-07-16 16:44 +09:00 |
| Clean-copy reproduction | PASS | `pnpm clean:check` | 352 source files; frozen offline install and all 11 command groups, including production build, Chrome E2E, demo replay, and evidence generation, pass | 2026-07-16 18:03 +09:00 |
| Static container contract | PASS | `pnpm container:check` | schema-v12 markers distinguish host-owned committed barrier slots and publish-last release, the non-privileged lifecycle, and helper source/protocol/client from all still-false Docker integration, runtime, finalization, PASS, and live flags | 2026-07-16 18:03 +09:00 |
| Dynamic container health | FAIL | `pnpm container:verify` | immutable Node 22.22.2 base is unset, so Docker build/runtime/SQLite restart checks did not run | 2026-07-16 16:44 +09:00 |
| Dynamic worker/verifier smoke | FAIL | `pnpm worker:verify` | schema v12 and role hashes match; fails before Docker at the unset immutable Node base with no runtime evidence | 2026-07-16 16:44 +09:00 |
| Dynamic TLS-only egress smoke | FAIL | `pnpm egress:verify` | schema v12 and role hashes match; fails before Docker at the unset immutable Node base; outbound remains `NOT_MEASURED` | 2026-07-16 16:44 +09:00 |
| Secret scan | PASS | credential-shaped `rg` scan | no matches | 2026-07-14 08:20 +09:00 |
| Dependency/license review | FAIL | `pnpm license:check`; prior `pnpm audit --prod --json` | 6 production dependencies inventoried, audit 0 vulnerabilities, NOTICE present; owner-selected project LICENSE absent with `OWNER_DECISION_REQUIRED` | 2026-07-16 16:59 +09:00 |
| Security review | PASS | `pnpm security:check` | 352 files/323 text files plus Git history scanned; no findings | 2026-07-16 18:03 +09:00 |
| Submission consistency | FAIL | `pnpm submission:check` | exactly 29 unmet requirements; schema-v12 prepared-boundary notes remain non-final and no fabricated URL, video, license, confirmation, or Codex capture exists | 2026-07-16 18:03 +09:00 |
...
- `pnpm verify`: `FAIL_EXPECTED; every implemented post-fix schema-v12 gate passed and only the owner LICENSE plus the exact 29-item non-final submission gate failed`
- `pnpm verify:live`: `FAIL_CLOSED_BEFORE_NETWORK; OPENAI_API_KEY and CODEX_MODEL are absent, and immutable role images, real-Docker/cumulative-CPU/outbound observations, fresh GPT/Codex evidence, and live wiring do not exist`
