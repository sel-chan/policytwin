...
- The installed Codex CLI 0.144.0 reports an existing ChatGPT login, and the repository pins `@openai/codex-sdk` plus bundled CLI 0.144.3. After owner approval, one attempt failed before model use on login-stream parsing; a second reached the bounded Codex phases but failed closed before evidence promotion because Windows retained the isolated temporary Codex state database. No partial challenge evidence was promoted.
