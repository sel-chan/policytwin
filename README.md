...
Schema v12 retains the non-live observer and private final-result guard, then adds three explicitly non-admitted building blocks. A host/role one-shot barrier protocol pre-creates host-owned receipt payload and SHA-256 commit slots inside a non-replaceable directory, separates them from a container-read-only host release, guards concurrent release calls, and locks `NODE_OPTIONS`; its POSIX permission design lets a non-root role fill only those existing slots while the host retains ownership for later snapshotting. The trusted bundled launcher closes the payload before committing its hash. Actual cross-UID bind-mount behavior, launcher process/FD exclusivity, and Docker integration remain dynamically unverified release blockers. The launcher is bundled in all role images but is not used by the static Docker driver. A `NON_PRIVILEGED_TEST_PORT` lifecycle harness exercises exact barrier/baseline order, serial per-role RAW/sample contracts, quiescent teardown-tail accounting, cancellation, containment diagnostics, and bounded cleanup termination while returning only frozen non-final/non-live results; if forced termination cannot settle an operation, it returns no result at all. A repository-owned C helper plus private client defines fixed binary frames and Linux RAW clock, pidfd, `openat2`, cgroup v2 identity, freeze, kill, quiescence, and release primitives without adding an in-process FFI dependency. The latest source compiles strictly in local WSL2, but that WSL cgroup root is tmpfs and runtime initialization therefore fails closed. No Docker-owned system adapter, reproducible pinned helper image, finalized-evidence issuer, signer admission, PASS path, or live claim exists.
