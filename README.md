...
Schema v11 retains the non-live observer controls introduced in v10 and adds a separate internal CPU-adapter capability scaffold plus a 28-stage dedicated success-order contract. Compile-time brands are backed by module-private object identity; plain objects, copies, wrappers, synthetic candidates, raw CPU evidence, and the non-live observer cannot satisfy finalized-evidence admission. The required order holds every role behind a start barrier until cgroup binding and baseline, then requires Docker/cgroup cleanup and controller stop before evidence finalization. This is only a fail-closed contract: no finalized-evidence issuer, Linux adapter, barrier runtime, RAW clock, polling, containment, signer admission, or PASS path exists.
