...
This proves the local transport and supervisor boundary only. The integration executor returns an explicitly labeled signed `FAIL` test result and performs no SDK work. Separate non-root worker and verifier Dockerfiles, an exact fixture/two-file-overlay execution plan, credential-free verifier environment, fixed resource arguments, and fail-closed dynamic smoke command now exist as static configuration. Their immutable image digests are unset and Docker has not run them; the OpenAI-only egress proxy, real OS executor, cgroup/process-tree teardown proof, and live receipt remain absent. No live PASS response or Codex repair exists, so `pnpm verify:live` remains fail-closed.
