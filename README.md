...
The repository now also defines a contract-only Worker RPC v2 candidate envelope for a future live Linux CPU result. It has a separate protocol, Ed25519 signature domain, TLS ALPN/frame magics, structured `LIVE_LINUX_CGROUP_RPC_V2` trust entries, key material distinct from v1, mutual TLS only, and a durable SQLite replay requirement. A client-derived execution binding and a Docker binding recomputed from the request plus exact container/PID/start/cgroup identities cover the immutable image, policy, accepted corpus, final tree, ordered egress/worker/verifier role samples, aggregate arithmetic, controller stop, and cgroup release. The client shape requires a strict proof for `PASS` and `cpuProof:null` for `FAIL`, but the repository's generic v2 supervisor is deliberately fail-only until a real controller is wired; executor-injected signer fields, reused key material, v1/static/unsigned input, replay, and downgrade are rejected. This is parser/signature/loopback-contract evidence only: synthetic PASS proofs use test keys, the loopback v2 integration signs explicit `FAIL`, and the live gate remains closed. The current candidate proof still lacks a signed global timestamped event transcript and signed CPU-failure/containment union, so those are explicit blockers before PASS signing. No Linux controller, Docker run, `cpu.stat` observation, model call, or Codex repair is claimed, and hard-limit plus bounded-overshoot flags remain false.
