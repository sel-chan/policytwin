...
This proves the local transport and supervisor boundary only. The integration executor returns an explicitly labeled signed `FAIL` test result and performs no SDK work. There is still no separately built non-privileged worker image, fixture-only mount, OpenAI-only egress proxy, no-network verification compartment, externally enforced CPU/memory/PID policy, or process-tree teardown receipt from a live run. No live PASS response or Codex repair exists, so `pnpm verify:live` remains fail-closed.
