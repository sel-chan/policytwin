...
Schema v14 retains the non-live observer, private final-result guard, one-shot barrier, non-privileged lifecycle harness, and fixed-frame native helper/client boundary. It now requires one deeply frozen, factory-issued supervisor lifecycle plan; copied or caller-shaped plans cannot configure the private runtime. The Docker owner derives all role plans only after it has created and independently inspected the exact worker-internal and outbound networks, then creates and reobserves containers, issues single-use bind/removal receipts, and requires container-name plus network absence before cleanup can complete. A create command that throws, times out, or returns malformed output is treated as side-effect-ambiguous and can recover only one exact-name, exact-label owned resource through an independent cleanup signal; an empty, foreign, or ambiguous observation leaves the run poisoned. The full helper role session, private Docker/cgroup system adapter, and dedicated lifecycle keep each role behind its barrier while Docker is observed, helper bind captures the initial CPU value, Docker is reobserved, that cached baseline is accepted, and the barrier is released. Serial sampling, containment, and quiescent final sampling follow. On the normal path each role's Docker resource is removed before its cgroup is released; after all roles, the helper stops and the owned networks are removed before the barrier and owner close. The emergency path instead requires all Docker roles and networks absent before bounded helper termination. The helper source compiles strictly in local WSL2, but the available cgroup root is tmpfs, no Linux Docker/cgroup-v2 runtime has exercised this path, and the static Docker gate still uses its older entrypoints. Cross-UID barrier permissions, surviving writable FDs, helper packaging, runtime containment, and cleanup therefore remain unverified. No finalized-evidence issuer, signer admission, PASS path, or live claim exists.
